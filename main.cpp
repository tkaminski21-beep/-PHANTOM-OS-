...
void input_update() {
    ...
    if (M5Cardputer.Keyboard.isChange() && M5Cardputer.Keyboard.isPressed()) {
       ...
       // Pobieranie wpisanego znaku
        if (!status.word.empty()) {
            LastChar = status.word[0]; // Pierwszy naciśnięty znak
        }

        // Obsługa Entera (Wybór)
        if (status.enter) SelPress = true;

        // Obsługa nawigacji (używając LastChar)
        if (LastChar == ';' || LastChar == ',') PrevPress = true;
        if (LastChar == '.' || LastChar == '/') NextPress = true;

        // Obsługa wyjścia (Esc / Przycisk boczny / Tylda)
        if (M5Cardputer.BtnA.wasPressed() || LastChar == '`') EscPress = true;
    }
}
